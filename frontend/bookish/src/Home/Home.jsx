...
const Home = () => {
  ...
  const getNot = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/notifications/${persdata}`
      );
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/profiles/${persdata}`
      );
      console.log(response.data);
      ...
      <div className="anunturi-form-home-Container">
        <div className="altcv">
          <h1 className="home-message">
            Welcome back:{" "}
            <span className="different-font">{info && info.username}</span>
          </h1>
        ...
        <div className="notificari">
          ...
          {data &&
            data.map((item) => <NotifDetail key={item.id} data={item} />)}
        </div>
      </div>
    </div>
  ) : (
